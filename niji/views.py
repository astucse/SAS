    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse('topic', kwargs={'pk': topic_id})
            )
...
def search_redirect(request):
    if request.method == 'GET':
        ...
        return HttpResponseRedirect(reverse('search', kwargs={'keyword': keyword}))
    else:
...
def create_topic(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            t = form.save()
            return HttpResponseRedirect(reverse('topic', kwargs={'pk': t.pk}))
    else:
        form = TopicForm()
...
def create_appendix(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('topic', kwargs={'pk': topic.pk}))
    else:
        form = AppendixForm()
